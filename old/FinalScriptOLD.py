#!/usr/bin/env python3
'''
##_DO_NOT_USE_THIS_ONE!_##
This is the first version used for testing. 
It will be archived just incase we need to look over it again
This version only prints to the command line so it could be used for testing IF updated!
'''
# Import modules used for this script
import subprocess
import socket

# Uses 'socket' to make connection and grab ip address
# s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
# s.connect(("8.8.8.8", 80))
# STUVMIP = s.getsockname()[0]
...
with open('output.txt', 'w') as f:
    subprocess.run("ls -l", stdout=f, text=True, shell=True, errors='ignore')